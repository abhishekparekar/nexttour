  ...
  return (
    <div className="bg-gray-50 min-h-screen">
      <div className="bg-white border-b border-gray-100 px-4 py-3 flex flex-wrap items-center justify-between gap-2">
        <div>
          <h1 className="text-base font-bold text-gray-900">Payment Management</h1>
          <p className="text-gray-600 text-xs mt-0.5">{bookings.length} active bookings tracked</p>
        </div>
      </div>
        ...
        <div className="bg-white rounded-xl border border-gray-100 shadow-sm overflow-hidden">
                  ...
                  return (
                    <tr key={booking.id} className="border-b border-gray-100 last:border-0 hover:bg-gray-50 transition-colors">
                      ...
                      <td className="px-3 py-2 align-middle">
                        ...
                        <div className="text-gray-600 text-xs mt-0.5">{booking.selectedDate}</div>
                      </td>
                      ...
                      <td className="px-3 py-2 align-middle">
                        <span className={`px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${paymentStatusColors[booking.paymentStatus || 'pending']}`}>
                        ...
                        {booking.payments && booking.payments.length > 0 ? (
                          <div className="space-y-1">
                            {booking.payments.map((txn, index) => (
                              <div key={txn.id || index} className="text-xs text-gray-500 flex items-center gap-1 bg-gray-50 p-1 rounded border border-gray-100">
                ...
                <div key={booking.id} className="p-4 space-y-3">
                  <div className="flex justify-between items-start">
                    <div>
                      ...
                      <p className="text-gray-600 text-xs mt-0.5">{booking.phone}</p>
                    </div>
                    <span className={`px-2 py-0.5 rounded-full text-xs font-bold uppercase tracking-wider ${paymentStatusColors[booking.paymentStatus || 'pending']}`}>
                      {booking.paymentStatus || 'pending'}
                    </span>
                  ...
                  <div className="bg-gray-50 rounded-xl p-3 text-xs space-y-1.5 border border-gray-100">
                    ...
                    {booking.payments && booking.payments.length > 0 && (
                      <div className="border-t border-gray-200/60 pt-2 mt-2">
                        <span className="text-xs text-gray-400 uppercase tracking-wider block mb-1">Receipts Log</span>
                        <div className="flex flex-wrap gap-1.5">
                          {booking.payments.map((txn, index) => (
                            <span key={txn.id || index} className="text-xs bg-white border border-gray-200 px-1.5 py-0.5 rounded text-gray-600 font-semibold">
                              ₹{txn.amount} ({txn.mode})
                            </span>
                          ))}
                        </div>
          ...
          {filteredBookings.length === 0 && (
            <div className="text-center py-12">
              ...
              <p className="text-gray-600 text-sm">Create bookings or adjust filters to list payment histories.</p>
            </div>
          )}
        </div>
      </div>
          ...
          <div className="bg-white rounded-3xl w-full max-w-md overflow-hidden shadow-2xl animate-in zoom-in-95 duration-200">
            <div className="px-6 py-4 border-b border-gray-150 flex items-center justify-between bg-gray-50/50">
              <div>
                ...
                <p className="text-gray-600 text-xs">Customer: {selectedBooking.name}</p>
              </div>
