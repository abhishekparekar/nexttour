    ...
    const unsubscribeTrips = useCachedTrips((allTrips) => {
      ...
      const matchedTrips = allTrips.filter(trip => {
        ...
        return (
          tripCatId === catId ||
          tripCatName === catId ||
          (catTitle && (tripCatId === catTitle || tripCatName === catTitle))
        );
      });
      ...
      <div className="relative h-[40vh] min-h-[300px] overflow-hidden rounded-b-3xl shadow-md">
        ...
        <div className="absolute inset-0 flex items-end">
          <div className="max-w-7xl mx-auto w-full px-4 sm:px-6 lg:px-8 pb-8">
            <nav className="flex items-center gap-1.5 text-xs text-gray-300 font-semibold mb-3">
            ...
            <div className="flex flex-col sm:flex-row sm:items-end justify-between gap-4">
              <div>
                <span className="inline-block px-3 py-1 bg-[#00C9B7]/20 text-[#00C9B7] rounded-full text-xs font-bold mb-3 backdrop-blur-sm">
                  Featured Category
                </span>
                <h1 className="text-3xl md:text-4xl lg:text-5xl font-black text-white mb-2">{categoryTitle}</h1>
                <p className="text-gray-200 text-sm max-w-xl font-medium">
              ...
              <div className="flex gap-5 bg-white/15 backdrop-blur-md rounded-xl px-5 py-3 border border-white/20 flex-shrink-0">
                <div className="text-center">
                  ...
                  <div className="text-xs text-gray-300 font-medium">Trips</div>
                </div>
                <div className="w-px bg-white/20" />
                <div className="text-center">
                  <div className="text-2xl font-bold text-white">{filteredTrips.filter(t => t.featured).length}</div>
                  <div className="text-xs text-gray-300 font-medium">Featured</div>
                </div>
              </div>
            </div>
          </div>
